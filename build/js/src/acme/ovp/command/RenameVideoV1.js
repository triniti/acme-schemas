// @link http://acme-schemas.triniti.io/json-schema/acme/ovp/command/rename-video/1-0-0.json#
import GdbotsNcrRenameNodeV1Mixin from '@gdbots/schemas/gdbots/ncr/mixin/rename-node/RenameNodeV1Mixin';
import GdbotsPbjxCommandV1Mixin from '@gdbots/schemas/gdbots/pbjx/mixin/command/CommandV1Mixin';
import GdbotsPbjxCommandV1Trait from '@gdbots/schemas/gdbots/pbjx/mixin/command/CommandV1Trait';
import Message from '@gdbots/pbj/Message';
import MessageResolver from '@gdbots/pbj/MessageResolver';
import Schema from '@gdbots/pbj/Schema';
import TrinitiOvpRenameVideoV1Mixin from '@triniti/schemas/triniti/ovp/mixin/rename-video/RenameVideoV1Mixin';

export default class RenameVideoV1 extends Message {
  /**
   * @private
   *
   * @returns {Schema}
   */
  static defineSchema() {
    return new Schema('pbj:acme:ovp:command:rename-video:1-0-0', RenameVideoV1,
      [],
      [
        GdbotsPbjxCommandV1Mixin.create(),
        GdbotsNcrRenameNodeV1Mixin.create(),
        TrinitiOvpRenameVideoV1Mixin.create(),
      ],
    );
  }
}

GdbotsPbjxCommandV1Trait(RenameVideoV1);
MessageResolver.register('acme:ovp:command:rename-video', RenameVideoV1);
Object.freeze(RenameVideoV1);
Object.freeze(RenameVideoV1.prototype);
